...
class StudentAdmission(models.Model):
    ...
    # الكود الجديد الذي سيحل محل القديم
    ...
    def _check_valid_time(self):
        for record in self:
            try:
                ...
                # New Validation: Allow overnight reservations, but disallow zero-duration sessions
                if start_time == end_time:
                    raise ValidationError("Start time and end time cannot be the same for a reservation.")
    ...
    def action_enroll(self):
        ...
        for record in self:
            ...
            for current_date in weekday_schedule[:record.n_of_reservations]:
                # نفس الكود الجديد من التعديل الثاني
                # 1. إنشاء وقت البدء كالمعتاد
                start_datetime = datetime.combine(current_date, start_time)

                # 2. إنشاء وقت انتهاء مؤقت في نفس اليوم
                temp_end_datetime = datetime.combine(current_date, end_time)

                # 3. التحقق: إذا كان وقت الانتهاء أقل من أو يساوي وقت البداية، فهذا يعني أنه في اليوم التالي
                if end_time <= start_time:
                    end_datetime = temp_end_datetime + timedelta(days=1)
                else:
                    # وإلا، فهو في نفس اليوم
    ...
    def action_open_renewal_wizard(self):
        ...
        self.ensure_one()
        view_id = self.env.ref('bi_health_care_center_management.admission_renewal_wizard_form').id
        ...
        # إضافة هذه السطور لنقل الأوقات أيضاً
        context['default_start_time'] = self.start_date
        context['default_end_time'] = self.end_date
        context['default_city_distance_id'] = self.city_distance_id.id
        context['default_driver_id'] = self.driver_id.id
        # إذا كان لديك باقة، قد ترغب في نقلها أيضاً
        context['default_package_id'] = self.package_id.id
        context['default_package_line_id'] = self.package_line_id.id
        context['default_n_of_reservations'] = self.n_of_reservations
