...
class AccountInvoice(models.Model):
    ...
    discount_type = fields.Selection([('percent', 'Percentage'), ('amount', 'Amount')], string='Discount type',
                                     ...
                                     default='percent')
    discount_rate = fields.Float('Discount Rate', digits=(16, 20),
...
class CustomAccountTax(models.Model):
    ...
    def _prepare_tax_totals(self, base_lines, currency, tax_lines=None):
        tax_totals = super(CustomAccountTax, self)._prepare_tax_totals(base_lines, currency, tax_lines)

        discount_rate = self.env.context.get('discount_rate', 0.0)

        # Calculate the new amount_total
        amount_untaxed = tax_totals.get('amount_untaxed', 0.0)
        amount_tax = tax_totals.get('amount_tax', 0.0)
        amount_total = amount_untaxed + amount_tax - discount_rate

        # Update the tax_totals dictionary
        tax_totals['amount_total'] = currency.round(amount_total) if currency else amount_total
