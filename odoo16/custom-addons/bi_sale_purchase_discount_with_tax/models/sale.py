...
class sale_order(models.Model):
			...
			if res_config.tax_discount_policy:
				if res_config.tax_discount_policy == 'tax':
					...
					if order.discount_type == 'line':
						order.discount_amt = 0.00 
						order.update({
							'amount_untaxed': amount_untaxed -applied_discount,
							'amount_tax':  order.amount_tax,
							'amount_total': amount_untaxed + order.amount_tax - applied_discount,
	...
	amount_discount = fields.Float('Amount Discount', compute='_compute_amount_discount_amount', store=True)
	tax_totals_amount = fields.Float('Tax Totals Amount', compute='_compute_tax_totals_amount', store=True)	
	
	@api.depends('discount_amt')
	def _compute_amount_discount_amount(self):
		for record in self:
			record.amount_discount = record.discount_amt
