...
var pdfPreview = core.Class.extend({
    ...
    preview: function(url) {
        ...
        var urlTemplate = _.template("/pdf_print_preview/static/lib/pdfjs/web/viewer.html?file=<%= url %>");
        var fullUrl = urlTemplate({url: url});  // Generate the full URL using the template
        console.log("Generated PDF Viewer URL:", fullUrl);  // Log the full URL to the console
        result.resolve($(QWeb.render("PDFjsFrame", {url: fullUrl})));
        return $.when(result);
    }
