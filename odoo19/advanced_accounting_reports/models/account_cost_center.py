...
class AccountCostCenter(models.Model):
    ...
    name = fields.Char(string='Cost Center', required=True, translate=True)
    ...
    child_ids = fields.One2many('account.cost.center', 'parent_id', string='Sub Cost Centers')
    ...
    def fields_get(self, allfields=None, attributes=None):
        fields = super(AccountCostCenter, self).fields_get(allfields, attributes)
        try:
    ...
    def _compute_debit_credit_balance(self):
        ...
        for center in self:
            company = center.company_id or self.env.company
            currency = company.currency_id
            center.debit = 0.0
            center.credit = 0.0
            center.balance = 0.0
            center.currency_id = currency
            center.secondary_debit = 0.0
            center.secondary_credit = 0.0
            center.secondary_balance = 0.0
            center.secondary_currency_id = False
            center.avg_rate = 0.0
            ...
            try:
                ...
                center.secondary_currency_id = sec_currency
            except Exception:
                pass
            ...
            try:
                ...
                center.secondary_debit = sdr
                center.secondary_credit = scr
                center.secondary_balance = sdr - scr
                ...
                if not float_is_zero(dr, precision_rounding=rounding) or not float_is_zero(cr, precision_rounding=rounding):
                    ...
                    if not float_is_zero(tp, precision_rounding=rounding):
                        center.avg_rate = float_round(ts / tp, precision_digits=6)
            except Exception:
                pass
