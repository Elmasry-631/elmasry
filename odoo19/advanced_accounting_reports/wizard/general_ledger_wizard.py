...
class GeneralLedgerWizard(models.TransientModel):
    ...
    @api.depends('date_from', 'date_to', 'journal_ids', 'account_ids', 'partner_ids', 'feature_ids', 'cost_center_ids', 'target_move', 'company_id')
    def _compute_balances(self):
        for wizard in self:
            ...
            if wizard.partner_ids:
                period_domain.append(('partner_id', 'in', wizard.partner_ids.ids))
    ...
    def action_view_report(self):
        ...
        return {
            ...
            'search_view_id': self.env.ref('advanced_accounting_reports.view_general_ledger_report_search').id,
            'views': [
                (self.env.ref('advanced_accounting_reports.view_general_ledger_report_list').id, 'list'),
            ],
        }
    ...
    def action_export_excel(self):
        ...
        return {
            ...
            'url': '/advanced_accounting/export/general_ledger_xlsx?wizard_id=%s' % self.id,
            'target': 'self',
        }

    def action_export_pdf(self):
        self.ensure_one()
        return self.env.ref('advanced_accounting_reports.action_report_general_ledger_pdf').report_action(self)
