...
class AccountMove(models.Model):
    ...
    def action_send_whatsapp(self):
        ...
        invoice_url = f"{base_url}/my/invoices/{self.id}?access_token={self.access_token}"


        default_message = (
            f"Hi {self.partner_id.name},\n"
            f"Here is your invoice {self.name} amounting {self.amount_total} {self.currency_id.symbol} "
            f"from {self.company_id.name}. Please remit payment at your earliest convenience.\n"
            f"Please use the following communication for your payment {self.name}.\n"
    ...
    def action_whatsapp_multi(self):
        ...
        if partner_check:
            ...
            message = ("Hi" + " " + self.partner_id.name + ',' + '\n' +
                       ...
                       "invoice link : " + account_move_ids.invoice_url)
            ctx.update({
                'default_message': message,
                'default_partner_id': account_move_ids[0].partner_id.id,
                'default_mobile': account_move_ids[0].partner_id.phone,
