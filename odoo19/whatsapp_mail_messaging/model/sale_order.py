...
class SaleOrder(models.Model):
    ...
    def action_send_whatsapp(self):
        ...
        order_url = self.get_portal_url()
        if not order_url.startswith("http"):
            order_url = base_url.rstrip("/") + "/" + order_url.lstrip("/")
        default_message = (
            f"Hi {self.partner_id.name},\n"
            f"Your quotation {self.name} amounting {self.amount_total} {self.currency_id.symbol} "
            "is ready for review. Do not hesitate to contact us if you have any questions.\n"
            f"Quotation link: {order_url}"
        )
